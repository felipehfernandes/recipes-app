...
export default function DrinkDetail({ id }) {
  ...
  useEffect(() => {
    ...
    if (recipe.idMeal) {
      const recipeDrink = {
        ...
        meals: {
          ...listInProgressRecipes.meals,
          [recipe.idMeal]: ingredientsTrue,
        },
      };
      saveInProgressRecipes(recipeDrink);
    }
  }, [ingredientsTrue]);
  ...
  useEffect(() => {
    ...
    setIngredients(ingredientValues.filter((ingredient) => ingredient !== ''));
    setMeasures(measureValues.filter((measure) => measure !== ' '));
  }, [recipe]);

  const isAlcoholic = recipe?.strAlcoholic === 'Alcoholic' ? '- Alcoholic' : '';

  const handleShare = () => {
    copy(`http://localhost:3000/meals/${recipe.idMeal}`);
    setShareRecipe(true);
  };
